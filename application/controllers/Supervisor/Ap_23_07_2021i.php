...
public function login_post()  
{
  ...
  $query2= $this->db->get_where(json_decode(TABLES)->table4, array('mobile_no' => $mobile_no , 'status' => '1'));
...
{   
  ...
  $que=$this->db->query("select * from tbl_admin where mobile_no='".$mobile_no."'");
  $quedata=$this->db->query("select * from tbl_restaurant_staff_registration where mobile_no='".$mobile_no."'and admin_id='$admin_id'");
  $master_user=$this->db->query("select * from master_user where mobile_no='".$mobile_no."'");
  $row = $que->num_rows();
  $row1 = $quedata->num_rows();
  $row2 = $master_user->num_rows();
...
public function sendEmail_Post(){
  try{
    ...
    $result=$this->Supervisor->getUserData($_POST['mobile_num']);
    if(!empty($result)){
      $this->mailData($result[0]);
    }else{
...
{        
    ...
    $email=base64_encode($data['user_email']);
    $mobile_no=base64_encode($data['mobile_no']);
    $now = base64_encode(date('Y-m-d H:i:s'));
    $body.='Forgot your password? 
    To reset your password, click on the below link:

      <a href="'.base_url().'Login/resetPassword/'.$email.'/'.$mobile_no.'/'.$now.'">click here</a>
