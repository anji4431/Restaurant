  ...
  if(!empty($mobile_no))
  {
   ...
   if(!empty($current_data))
   {
     ...
     { 
      ...
      $array_admin=array(array('user_type'=>'Admin','images'=>base_url().'RoleImages/'.$AdminData[0]['image']));
      if(!empty($multipleData)){
        foreach($multipleData as $role){
          //add_new
        $data1->user_type=$role['user_type'];
        $res2 = $this->Supervisor->manage_login_data($data1);     
        //endadd_new   
...
$data['message']='success';
$data['status']  ='1';
$data['roles']=array(array('user_type'=>'Admin','images'=>base_url().'RoleImages/'.$AdminData[0]['image']));
$data['term_condition'] =  $term_data[0]['term_condtion'];
array_push($result , $data);
$response->data = $data;
        ...
        $explode=explode(',',$user_type);
          if(count($explode)==1){
            ...
            'user_type'=>$explode[0]
              ...
              if($explode[0] !='Waiter'){
                    $staffData=$this->Supervisor->getStaffData($explode[0],$admin_id);
                     if(!empty($staffData)){
                      $data1->status ='0';
                  ...
                  $flagData=$this->Supervisor->checkStaffData($admin_id,$mobile_no);
                  if(!empty($flagData)){
                       $data1->status ='0';
