    ...
    public function KycList()
    {
        try
        {
            ...
            foreach ($KycList as $list) {
                ...
                $action .= '<div class="row text-center" style="width: max-content;margin-right: auto; margin-left: auto;"><a class="btn btn-sm btn-primary btn-xs" href="javascript:void(0)" title="Verify" onclick="verify_kyc(' . "'" . $list->admin_id . "'," . "'" . $status . "'" .')"style="height: 33px">'.$button.'</a></div>';
                $row[] = $action;
                $data[] = $row;
            }
            $output = array(
