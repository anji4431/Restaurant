  ...
  public function login_post()
  {
    try
    {
      ...
      }else
      {
             ...
             $staffUser=$this->Login_model->checkUserForStaff($usertype,$username);
             ...
             {          
                       if(isset($staffUser[0]['user_type']) && $staffUser[0]['user_type']=='KOT')
                       {
                            $salt         =$staffUser[0]['salt'];
                            $db_password  =$staffUser[0]['password'];
                            $password     =$this->encrypt->decode($db_password, $salt);
                         // print_r($password);exit;
                            if($password==$user_password)
                            {
                            $this->session->set_userdata('admin_id', $staffUser[0]['admin_id']);
                            $this->session->set_userdata('id', $staffUser[0]['id']);
                            $this->session->set_userdata('mobile_no'  , $staffUser[0]['mobile_no']);
                            $this->session->set_userdata('user_email'  , $staffUser[0]['email']);
                            $this->session->set_userdata('user_fullname' , $staffUser[0]['name']);
                            $this->session->set_userdata('user_role' , 3);
                            $this->session->set_userdata('user_type' , $staffUser[0]['user_type']);
                            $session_data = $this->session->all_userdata();
                            $arry=array('status'=>'1','message'=>'success');
                            $this->response($arry, 200);
                       }else
