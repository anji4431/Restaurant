        ...
        if(!empty($result))
          {
                        ...
                        $array_merge_recursive=array_unique(array_merge_recursive($notification_data,array_merge_recursive($custData,$adminData)));
...
public function getNearBySpots_post()
{
  ...
  {     
        ...
        $result=$this->Customer->getNearBySpots($city,$food_code);
        if(!empty($result))
        {
           foreach($result as $value)
           {
...
function checkUuid_post(){
  try{
  $uuid    =$_POST['uuid'];
  $ex=explode('-',$uuid);
  $res=$ex[0];
   // print_r(gettype($uuid));exit;
  if(!empty($uuid)){
          ...
          if(!empty($major)){
            ...
            else 
            {
              $res_id=$major;
                ...
                $admin_id=$res_id;
                $get_restaurant_data=$this->Customer->getRestaurantDetail($admin_id);
                $result=array();    
        if(!empty($get_restaurant_data))
        {
         foreach ($get_restaurant_data as $row)
           {
            ...
            $data['amenities'] =  explode(",", $row['amenities']);
            $data['verified'] =   $row['verified'];
            $data['imageList']     =explode(",", $row['imageList']);

            array_push($result,$data);
           } 
           echo json_encode(array('status'=>1,'message'=>'success','data'=>$result));exit();
         }
         else
         {
          echo json_encode(array('status'=>0,'message'=>'No record found.'));exit();
         }
          }else{
...
public function getTest_post()
{
  ...
  redirect("http://localhost:8080/Restaurant/index.php/customer/ApiTest/getNearSpotsByUuid");
  die;
  // echo base_url().'index.php/customer/ApiTest/getNearSpotsByUuid';
  // die;
  // http://localhost:8080/Restaurant/index.php/customer/ApiTest/getNearSpotsByUuid
//$ch = curl_init('http://localhost:8080/Restaurant/index.php/customer/ApiTest/getNearSpotsByUuid');
// $ch = curl_init(base_url().'index.php/customer/ApiTest/getNearSpotsByUuid');
