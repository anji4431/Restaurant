...
/*.........Add order for customer  restaurant  for Restaurant Api  ---- */
        ...
        {  
             ...
             for($i=0;$i<count($menu_item_array);$i++)
              ...
              }else
              {
                ...
                $notification_data=array_merge_recursive($custData,array_merge_recursive($notificationData,$adminData));
              $title ='OYLY';
              $message ='table No. '.$table_no.' has more item';
              foreach($notification_data as $notification)
                  {
                    $result=sendPushNotification($title,$message,$notification['notification_id']);
                    $array[]=array(
  ...
  public function customer_data_post()
   ...
   public function show_notification_by_count_post()
    ...
    $response =   new StdClass();
    $result       =  new StdClass();
    $waiterList=$this->Customer->getWaiterList();
    $string='';
    foreach($waiterList as $waiter){
      $string .= "'".$waiter['mobile_no']."'".','; 
    }
    $customer_mobile_no =$this->input->post('customer_mobile_no');
    $resdata1 = $this->Customer->check_total_count_notification($customer_mobile_no);
    $resdata2 = $this->Customer->check_total_count_notification2($customer_mobile_no,rtrim($string,','));
   ...
   /*.........notification list for staff in  Restaurant ---- */
     public function get_notification_list_for_order_post()
      {
        ...
        $get_notification_data2 = $this->Customer->get_notification_data2($customer_mobile_no,rtrim($string,','));
    ...
    {
       $resdata = $this->Customer->check_status_for_notification($check_status,$customer_mobile_no,rtrim($string,','));
       if($resdata){
              $data1->status ='1';
              $data1->message = 'success';
              array_push($result,$data1);
...
public function sendPushNtification_post()
{
  ...
  if ($push_type == 'topic') {
      ...
      $json = $push->getPush();
      $response = $firebase->send('dpsgfLzJRxM:APA91bESycWI0N1ya31XT-M-ZPQShoMkhGpYNNVyfcENlYU5BdD2GvYM9EGbqUT7_1H2d_EYsnZD9dmEAaxCT3euEKr_tqyGMpkjgKsSXU_rlfTTOYfYfjwaZiubH7vtKZhwsWPLE1ZY', $json);
