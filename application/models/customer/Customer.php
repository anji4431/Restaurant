...
{    
      ...
      function logout_customer_data($data)
    {
        ...
        $now = $data->logout_time;
         $this->db->query("UPDATE tbl_login_customer SET active_status='$active_status',logout_time='$now',notification_id='' where mobile_no='$cus_id' and device_id='$device_id'");
    }
    ...
    public function check_total_count_notification($customer_mobile_no)
     {
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT order_id from tbl_notification_by_staff where staff_mobile_no='".$customer_mobile_no."' and status='1' and user_type='customer' and date_time LIKE '%$now%'");
        return($q->num_rows());
     }
    public function check_total_count_notification2($customer_mobile_no)
     {
        ...
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT order_id from tbl_notification_by_customer where mobile_no='".$customer_mobile_no."' and status='1' and user_type='customer' and date_time LIKE '%$now%'");
        return($q->num_rows());
     }
    ...
    {  
       date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
      $q=$this->db->query("SELECT * from tbl_notification_by_staff where staff_mobile_no='$customer_mobile_no' and date_time and user_type='customer'LIKE '%$now%' order by date_time desc");
      // print_r($this->db->last_query());exit;
       return($q->result_array());
    }
    ...
    {  
      date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
      $q=$this->db->query("SELECT * from tbl_notification_by_customer where mobile_no='$customer_mobile_no' and  date_time and user_type='customer' LIKE '%$now%'order by date_time desc");
       return($q->result_array());
    }
     function check_status_for_notification($check_status,$customer_mobile_no,$user_type)     
    {   

      $this->db->query("UPDATE tbl_notification_by_staff SET status='0' where staff_mobile_no='$customer_mobile_no' and user_type='customer'");
     return  $this->db->affected_rows();
    }
     function check_status_for_notification2($check_status,$customer_mobile_no,$user_type)     
    {   

      $this->db->query("UPDATE tbl_notification_by_customer SET status='0' where mobile_no='$customer_mobile_no'and user_type='customer'");
     return  $this->db->affected_rows();
    }
...
public function getCustId($order_id,$admin_id,$customer_mobile_no)
{
 ...
 function getPrvOrderDate($admin_id){
  ...
  public function getRestaurantStaffNotification($admin_id)
  {
    ...
    //$this->db->GROUP_BY('mlu.device_id');
    $query=$this->db->get();
    // print_r($this->db->last_query());exit;
    $result=$query->result_array();
    return $result;
 ...
 public function getNearBySpots($city,$food_code)
  {
    $this->db->select('*');
    $this->db->from('spots');
    ...
    if($food_code==1 && !empty($food_code)){
      $this->db->where('food_code',1);
    }
    $this->db->where('status',1);
    $query=$this->db->get(); 
    //print_r($this->db->last_query());
...
function getAdminData($admin_id){
  ...
  $this->db->where('a.user_type','Admin');
...
function checkLoginStatus($mobile_no,$device_id,$cus_id){
    ...
    $this->db->from('tbl_login_customer');
    $this->db->where('mobile_no',$mobile_no);
    //$this->db->where('notification_id',NULL);
    $query=$this->db->get();
    //print_r($this->db->last_query());exit;
    $result=$query->result_array();
    return $result;
...
function getRestaurantDetail($admin_id){
  $this->db->select('admin_id,city,spotId,trending,name,image,rating,lat,lng,location,cuisines,priceLevel,cost,openStatus,openingTime,closingTime,phone,address,amenities,verified,imageList');
  $this->db->from('spots');
  $this->db->where('admin_id',$admin_id);
  $this->db->where('status',1);
  $query=$this->db->get();
...
public function getNearSpotsByUuid($adminids)
{
  $this->db->select('*');
  $this->db->from('spots');
  $this->db->where_in('admin_id', $adminids);
  $this->db->where('status',1);
  $query=$this->db->get(); 
  //print_r($this->db->last_query());
  $result=$query->result_array();
