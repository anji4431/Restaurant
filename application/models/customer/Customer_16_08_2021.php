...
function getRestaurantDetail($admin_id){
  $this->db->select('admin_id,city,spotId,trending,name,image,rating,lat,lng,location,cuisines,priceLevel,cost,openStatus,openingTime,closingTime,phone,address,amenities,verified');
  $this->db->from('spots');
  $this->db->where('admin_id',$admin_id);
  $this->db->where('status',1);
  $query=$this->db->get();
