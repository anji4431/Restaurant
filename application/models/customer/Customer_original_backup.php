    ...
    public function check_total_count_notification($customer_mobile_no)
     {
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT order_id from tbl_notification_by_staff where staff_mobile_no='".$customer_mobile_no."' and status='1'");
        return($q->num_rows());
     }
    public function check_total_count_notification2($customer_mobile_no)
     {
        ...
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT order_id from tbl_notification_by_customer where mobile_no='".$customer_mobile_no."' and status='1'");
        return($q->num_rows());
     }
      function get_notification_data($customer_mobile_no)
    {  
      $q=$this->db->query("SELECT * from tbl_notification_by_staff where staff_mobile_no='$customer_mobile_no' order by date_time desc");
       return($q->result_array());
    }
    function get_notification_data2($customer_mobile_no)
    {  
      $q=$this->db->query("SELECT * from tbl_notification_by_customer where mobile_no='$customer_mobile_no' order by date_time desc");
       return($q->result_array());
    }
