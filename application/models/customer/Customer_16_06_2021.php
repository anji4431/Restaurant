    ...
    public function check_total_count_notification2($customer_mobile_no)
     {
        ...
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT order_id from tbl_notification_by_customer where customer_mobile_no='".$customer_mobile_no."'and mobile_no ='$customer_mobile_no' and status='1'");
        return($q->num_rows());
     }
      function get_notification_data($customer_mobile_no)
    {  
      $q=$this->db->query("SELECT * from tbl_notification_by_staff where customer_mobile_no='$customer_mobile_no' and staff_mobile_no='$customer_mobile_no' order by date_time desc");
       return($q->result_array());
    }
    function get_notification_data2($customer_mobile_no,$string)
    {  
      $q=$this->db->query("SELECT * from tbl_notification_by_customer where customer_mobile_no='$customer_mobile_no' and mobile_no='$customer_mobile_no' order by date_time desc");
       return($q->result_array());
    }
     function check_status_for_notification($check_status,$customer_mobile_no,$string)     
    {   

      $this->db->query("UPDATE tbl_notification_by_staff SET status='0' where customer_mobile_no='$customer_mobile_no'");
     return  $this->db->affected_rows();
    }
     function check_status_for_notification2($check_status,$customer_mobile_no,$string)     
    {   
