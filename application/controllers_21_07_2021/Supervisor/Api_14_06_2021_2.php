  ...
  {  
    ...
    if(!empty($orderResult))
    {
      $this->Supervisor->readyToserveOrder($order_id,$admin_id);
          ...
          $message               ='Table No '.$getCustmoerData[0]['table_no'].' Order Id '.$order_id2.' ready to serve';
          $notificationArray     =array(
                                'notification1'=>$custData[0]['notification_id'],
                                'notification2'=>$notification_id
                                 );
         // echo '<pre>';print_r($notificationArray);exit;
          foreach($notificationArray AS $value){
                              $i=1;
                              $result[]=sendPushNotification($title,$message,$value);
                              $i=$i+1;
                            }
...
{   
  ...
  {  
    ...
    // {
          ...
          $getCustmoerData          =$this->Supervisor->getCustmoerData($order_id,$admin_id);
          $message                  ='Table No '.$getCustmoerData[0]['table_no'].' Order Id '.$order_id2.' KoT created';
...
public function update_payment_for_customer_by_staff_post()
{
  ...
  {  
          ...
          $message            ='Order Id '.$order_id2.' amount has been paid.';
          $notification_data  =$this->Customer->getRestaurantStaffNotificationForPayment($admin_id);
...
public function get_notification_list_for_order_post()
{
  ...
  if($role=='Waiter'){
     ...
     $get_notification_data1 = $this->Supervisor->get_notification_data($staff_mobile_no);
     $get_notification_data2 = $this->Supervisor->get_notification_data3($staff_mobile_no);
     $get_notification_data  = array_merge_recursive($get_notification_data1,$get_notification_data2);
  }
...
public function update_sub_order_by_waiter_post()
{
  ...
  if($sub_order_status=='Rejected')
  {
    ...
    if(!empty($result))
    {
      ...
      $message            ='Sub Order No '.$sub_order_id.' ('.$waiterData[0]['name'].')'.' declined order';
      $result=sendPushNotification($title,$message,$staffData[0]['notification_id']);
      if(!empty($result))
                {
                  $array=array(
                    'send_from'=>$waiter_mobile_no,
                    'staff_mobile_no'=>$getCustmoerData[0]['customer_mobile_no'],
                    'admin_id'=>$admin_id,
                    'status'=>1,
                    'order_id'=>$order_id,
                    'sub_order_id'=>$sub_order_id,
...
else if($status==2 || $status==3 || $status==5)
{
 ...
 if(!empty($result))
 {
      ...
      $custData           =$this->Customer->getCustData($getCustmoerData[0]['customer_mobile_no']);
      $title              ='Sub Order Status';
      $message            ='Sub Order No '.$sub_order_id.' ('.$waiterData[0]['name'].')'.' Confirmed order';
      $result=sendPushNotification($title,$message,$staffData[0]['notification_id']);
      if(!empty($result))
                {
                  $array=array(
...
{   
  ...
  if($status==3 || $status==5)
  {
    ...
    if(!empty($result))
    {
          ...
          $staffData                =$this->Supervisor->getStaffNotification($admin_id,'Chef');
          $getCustmoerData          =$this->Supervisor->getCustmoerData($order_id,$admin_id);
          $message                  ='Table No '.$getCustmoerData[0]['table_no'].'Sub Order Id '.$sub_order_id.' KoT created';
  ...
  if(!empty($result))
  {
    ...
    if(empty($inv_result))
    {
      ...
      if($insetedId)
      {
            ...
            $order_date                 =date('Y-m-d',strtotime($this->Supervisor->getOrderDate($order_id)));
            $order_id2                  =str_replace(str_replace('_','',$admin_id),$order_date,$order_id);
            $title                      ='Please check Invoice';
            $message                    ='Order Id '.$order_id2.' Invoice created successfully.';
            $custData                   =$this->Supervisor->getCustData($getCustmoerData[0]['customer_mobile_no']);
            $result=sendPushNotification($title,$message,$custData[0]['notification_id']);
                if(!empty($result))
                {
                  $array=array(
   ...
   if(!empty($order_id)&&!empty($admin_id)&&!empty($id))
    {
      ...
      if(!empty($result2))
      {
        ...
        if(!empty($result))
        {
          ...
          $waiterData         =$this->Supervisor->getwaiterData($waiter_mobile_no);
          $staffData          =$this->Supervisor->getStaffNotification($admin_id,'Waiter');
          $getCustmoerData    =$this->Supervisor->getCustmoerData($order_id,$admin_id);
          $order_date         =date('Y-m-d',strtotime($this->Supervisor->getOrderDate($order_id)));
          $order_id2          =str_replace(str_replace('_','',$admin_id),$order_date,$order_id);
          $title              ='Order Status';
          $message            ='Table No '.$getCustmoerData[0]['table_no'].' ('.$waiterData[0]['name'].')'.'item declined';
          $result=sendPushNotification($title,$message,$staffData[0]['notification_id']);
                if(!empty($result))
                {
                  $array=array(
...
public function discountForCustomer_post()
{
    try
    {
      ...
      if(!empty($result))
      {
          ...
          $message            ='Supervisor'.'('.$waiterData[0]['name'].')'.$order_id2.' of '.$discount.'%';
