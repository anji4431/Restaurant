    ...
    public function getStaffList()
    {
     ...
     //      $row[]  = (!empty($list->password)&& !empty($list->salt))?$this->encrypt->decode($list->password,$list->salt):'';
          ...
          $row[]  =$list->permanent_address;
					...
					$row[] 	='<div class="row text-center" style="width: max-content;margin-right: auto; margin-left: auto;"><a class="btn btn-sm btn-primary btn-xs" href="javascript:void(0)" title="Edit" onclick="edit('."'".$list->mobile_no."'".')"style="height: 33px"><i class="glyphicon glyphicon-eye-open"></i>Edit</a>
				                  <a class="btn btn-info btn-xs" href="javascript:void(0)" title="'.$button.'" onclick="change('."'".$list->mobile_no."'".','."'".$list->status."'".')"style="height: 33px;"><i class="glyphicon glyphicon-saved"></i>'.$button.'</a>
				                  <a class="btn btn-danger btn-xs" href="javascript:void(0)" title="Delete" onclick="remove('."'".$list->mobile_no."'".')"style="height: 33px;display:none;"><i class="glyphicon glyphicon-deleted"></i>Delete</a></div>';
					$data[]	=$row;
				}
				$output = array(
								"draw" => $_POST['draw'],
...
public function change()
{
	try
	{
		...
		if(!empty($mobile_no))
		{
			...
			$result=$this->StaffList_model->change($mobile_no,($status==1?'0':'1'));
			if($result)
			{
				$arry=array('status'=>'1','message'=>'Success');
             	echo json_encode($arry, 200);exit();
...
public function getStaff()
{
	try
	{
		...
		if(!empty($mobile_no))
		{
			...
			$result=$this->StaffList_model->getStaff($mobile_no);
   ...
   public function updateStaff()
    ...
    if(!empty($resto_staff_document)){
      ...
      if(!empty($staff))
      {
        ...
        if(empty($superAdmin))
        {
             $result=$this->StaffList_model->updateStaff(($array+$img_array+$doc_array),$mobile_no);
              if($result)
              {
